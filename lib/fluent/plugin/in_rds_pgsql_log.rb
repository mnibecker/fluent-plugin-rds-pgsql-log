...
module Fluent
  class RdsPgsqlLogInput < Input
    Plugin.register_input('rds_pgsql_log', self)
    ...
    def configure(conf)
      ...
      raise ConfigError.new("region is required") unless @region
      if !has_iam_role?
        raise ConfigError.new("access_key_id is required") if @access_key_id.nil?
        raise ConfigError.new("secret_access_key is required") if @secret_access_key.nil?
      end
      raise ConfigError.new("db_instance_identifier is required") unless @db_instance_identifier
      raise ConfigError.new("pos_file is required") unless @pos_file
      raise ConfigError.new("refresh_interval is required") unless @refresh_interval
      raise ConfigError.new("tag is required") unless @tag
    end
      ...
      begin
        ...
        raw_records.each do |raw_record|
          ...
          unless line_match
            ...
            # emit before record
            Engine.emit(@tag, Engine.now, record) unless record.nil?
